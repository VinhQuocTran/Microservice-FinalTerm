...
function fetchRequestListingProcess(id) {
  return fetch(config.API_URL + "properties/" + id + "/getRequestListingProperty", {
    headers: {
...
export default function PropertiesTable(props) {
  ...
  const [backgroundCheckServices, setBackgroundCheckServices] = useState(null);
  ...
  useEffect(() => {
    const fetchBackgroundCheckServices = async () => {
      const response = await axios.get(
        config.API_URL + "services?serviceType=ST01"
      );
  ...
  const handleModalAction = async () => {
    if (modalAction === "update") {
      ...
      try {
        ...
        let prop = {
          ...
          note,
          selectedBackgroundCheckService,
        };

        await axios.post(
          config.API_URL + `properties/` + propertyData.id + "/verifyProperty",
          prop,
          {
            headers: {
              "Content-Type": "application/json",
    ...
    if (modalAction === "listing") {
      console.log(
        "Listing action with data:",
        selectedHouseInspection,
      );
      try {
        const jwtToken = localStorage.getItem("jwt");
        let prop = {
          propertyInspectionServiceId:
            listingData.houseInspection.serviceUsedId,
  ...
  return (
    ...
    >
      <Grid
        templateColumns={{ sm: "1fr", md: "1fr", lg: "10fr 2fr" }}
        mb="20px"
      >
        {/* Left column */}
        <Text
              ...
              <Tr {...row.getRowProps()} key={index}>
                {row.cells.map((cell, index) => {
                  ...
                  if (cell.column.id === "actions") {
                    data = (
                      ...
                      >
                        ...
                        >
                          <Button
                            onClick={() =>
                              onOpen("details", { id: row.original.id })
                            }
                            colorScheme="teal"
                            size="sm"
                            marginLeft="1"
                            ...
                            <ModalContent>
                              ...
                              <ModalBody>
                                {modalAction === "details" && modalData && (
                                  <Card>
                                    <Box>
                                      {Object.entries(propertyData).map(
                                        ([key, value]) =>
                                ...
                                {modalAction === "update" && (
                                  <div>
                                    ...
                                    <Select
                                      placeholder="Select background check service"
                                      value={selectedBackgroundCheckService}
                                      onChange={(e) =>
                                        setSelectedBackgroundCheckService(
                                          e.target.value
                                        )
                                      }
                                    >
                                      {backgroundCheckServices &&
                                        backgroundCheckServices.map((bcs) => (
                                          <option key={bcs.id} value={bcs.id}>
                                            {bcs.name} - {bcs.feePerTime}
                                          </option>
                                        ))}
                                    </Select>
                                      ...
                                      <Flex justify="space-between">
                                        ...
                                        >
                                          ...
                                          <Text
                                            ...
                                            mb="15px"
                                          >
                                            {listingData.property.accountId}
                                          </Text>
                                          <Text
                                            color={textColor}
                                            fontSize="16px"
                                            ...
                                            mb="15px"
                                          >
                                            {listingData.property.createdAt}
                                          </Text>
                                        </Box>
                                      </Flex>
                                      <Text
                          ...
                          <Button
                            onClick={() =>
                              onOpen("update", { id: row.original.id })
                            }
                          ...
                          <Button
                            onClick={() =>
                              onOpen("listing", { id: row.original.id })
                            }
                            isDisabled={
                              (row.original.isListed === "1" &&
                                row.original.isVerified === "1") ||
                              (row.original.isListed === "0" &&
                                row.original.isVerified === "0") ||
                              (row.original.isListed === "-1" &&
                                row.original.isVerified === "0") ||
                              (row.original.isListed === "-1" &&
                                row.original.isVerified === "1")
                    ...
                    data = <Image src={cell.value} height={"auto"}></Image>;
                  } else if (cell.column.id === "propertyDocumentUrl") {
                    data = (
                      <Link color={"brand.300"} href={cell.value}>
                        {" "}
                        Link here
                      </Link>
                    );
