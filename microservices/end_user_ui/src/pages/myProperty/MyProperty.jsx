...
} from "../../redux/userPropertiesSlice";
...
const MyProperty = () => {
  ...
  const [isNewPropertyModalOpened, setIsNewPropertyModalOpened] =
    useState(false);
  const [isVerifyPropertyModalOpened, setIsVerifyPropertyModalOpened] =
    useState(false);
  const [isEditPropertyModalOpened, setIsEditPropertyModalOpened] =
    useState(false);
  const [isListingPropertyModalOpened, setIsListingPropertyModalOpened] =
    useState(false);
  const [isInspectionSelectOpened, setIsInspectionSelectOpened] =
    useState(false);
  ...
  const [selectedInspectionService, setSelectedInspectionService] =
    useState(null);
  const [selectedValuationService, setSelectedValuationService] =
    useState(null);
  ...
  const handleNewPropertyModalClick = (e) => {
    newPropertyModalRef.current.style.visibility = !isNewPropertyModalOpened
      ? "visible"
      : "hidden";
    newPropertyModalRef.current.style.opacity = !isNewPropertyModalOpened
      ? 1
      : 0;
  ...
  const handleVerifyPropertyClick = (e) => {
    verifyPropertyModalRef.current.style.visibility =
      !isVerifyPropertyModalOpened ? "visible" : "hidden";
    verifyPropertyModalRef.current.style.opacity = !isVerifyPropertyModalOpened
      ? 1
      : 0;
  ...
  const handleEditPropertyModalClick = (e) => {
    if (!isEditPropertyModalOpened) {
      const property = currentUserProperties.userProperties.find(
        (item) => item.id === e.target.dataset.id
      );
      setEditProperty(property);
      setEditValues(property);
      setEditDistrict({
        label: property.propertyDistrict,
        value: property.propertyDistrict,
      });
    }
    editPropertyModalRef.current.style.visibility = !isEditPropertyModalOpened
      ? "visible"
      : "hidden";
    editPropertyModalRef.current.style.opacity = !isEditPropertyModalOpened
      ? 1
      : 0;
  ...
  const handleListingPropertyModalClick = (e) => {
    listingPropertyModalRef.current.style.visibility =
      !isListingPropertyModalOpened ? "visible" : "hidden";
    listingPropertyModalRef.current.style.opacity =
      !isListingPropertyModalOpened ? 1 : 0;
  ...
  const handleEditPropertySubmit = async (e) => {
    ...
    try {
      await axios.patch(
        BASE_URL + `/properties/${editProperty.id}`,
        Object.fromEntries(data),
        {
          headers: {
            Authorization: `Bearer ${currentUser.token}`,
      ...
      dispatch(
        updateProperty({ id: editProperty.id, ...Object.fromEntries(data) })
      );
  ...
  const handleAcceptBtnClick = async () => {
    try {
      const response = await axios.patch(
        BASE_URL + `/properties/${verifyPropertyId}`,
        {
          isVerified: "0",
        },
        {
  ...
  useEffect(() => {
    const fetchInspectionServices = async () => {
      const response = await axios.get(
        BASE_URL + "/services?service_type=ST03"
      );
      setInspectionServices(response.data.data);
    };

    const fetchValuationServices = async () => {
      const response = await axios.get(BASE_URL + "/services?service_type=ST02");
  ...
  useEffect(() => {
    const fetchCurrentUserProperties = async () => {
      ...
      try {
        const response = await axios.get(
          BASE_URL + `/properties?accountId=${currentUser.user.id}`
        );
    ...
    <div className={`myProperty`}>
      <ContentWrapper>
        <div className="buttons">
          <button type="button" onClick={handleNewPropertyModalClick}>
            Create new property
          </button>
        </div>
          ...
          {currentUserProperties.userProperties &&
            currentUserProperties.userProperties.map((item, index) => (
              <PropertyCard
                key={index}
                property={item}
                onClick={handleVerifyPropertyClick}
                onEditModalClick={handleEditPropertyModalClick}
                onListingPropertyClick={handleListingPropertyModalClick}
              />
            ))}
        </div>
      </ContentWrapper>
      <div
        className="newPropertyModal"
        ref={newPropertyModalRef}
        onSubmit={handleNewPropertySubmit}
        onClick={handleNewPropertyModalClick}
      >
      ...
      <div
        className="verifyPropertyModal"
        ref={verifyPropertyModalRef}
        onClick={handleVerifyPropertyClick}
      >
        <div className="modalContent" onClick={(e) => e.stopPropagation()}>
          <div className="contentTop">
          ...
          <div className="line"></div>
          <h4>
            Do you understand that all of your provided information about the
            property will be used for verification process?
          </h4>
          <div className="line"></div>
          <div className="submitBtn">
            <button type="button" onClick={handleAcceptBtnClick}>
              Accept
            </button>
            <button type="button" onClick={handleVerifyPropertyClick}>
              No
            </button>
          </div>
        </div>
      ...
      <div
        ...
        ref={editPropertyModalRef}
        onSubmit={handleEditPropertySubmit}
        onClick={handleEditPropertyModalClick}
      >
        <div className="modalContent" onClick={(e) => e.stopPropagation()}>
          <form action="">
            <div className="contentTop">
            ...
            <div className="inputForm">
              ...
              <Select
                ...
                options={districtOptions}
                value={editDistrict}
                onChange={(selectedOption) => setEditDistrict(selectedOption)}
                required
              />
      ...
      <div
        className="listingPropertyModal"
        ref={listingPropertyModalRef}
        onSubmit={handleListingPropertySubmit}
        onClick={handleListingPropertyModalClick}
      >
        <div className="modalContent" onClick={(e) => e.stopPropagation()}>
          <form action="">
            <div className="contentTop">
            ...
            <div className="contentBody">
              <div className="selectBox">
                <FilterItem
                  ...
                  selectedItem={selectedInspectionService}
                  setSelectedItem={setSelectedInspectionService}
                  setServicePrice={setInspectionServicePrice}
                  isNotChange={false}
                />
